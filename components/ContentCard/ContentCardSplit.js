...
const ContentCardSplit = ({id, image, alt, w, h, q}) => {
  return (
    <div id={id} className="relative w-11/12 m-auto mt-10 mb-10 flex flex-col lg:flex-row items-center justify-center "> 
    {/* Ramme */}
      <div className="bg-gray-400 w-full h-full lg:w-1/2 rounded-t-lg lg:rounded-tr-none lg:rounded-l-lg drop-shadow-lg">
        {/* Bilde-boks */}
        <div className="p-5 text-white font-bold">
            {/* Bilde */} 
            <Image src={`/images/${image}`} alt={alt} quality={q} layout='fill' />
        </div>
      </div>
